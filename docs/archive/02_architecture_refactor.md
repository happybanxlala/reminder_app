...
- `Milestone` occurrence 屬於 `Timeline`，只可 `noticed / skipped`，不會產生下一筆
...
- `timeline_milestone_rules` 是 milestone primary source
- milestone occurrence 不預生成，改由 `TimelineMilestoneService` 動態計算
- `timeline_milestone_records` 只保存 noticed / skipped / notified occurrence
...
### TimelineMilestoneService

- 計算 milestone reminder date
- 計算 next / today / upcoming occurrences
- 將 rule 計算結果與 persisted records 合併
...
- `Timeline` CRUD
- `Milestone Rule` CRUD
- `Milestone Record` status transition
- timeline lifecycle

### HomeQueryService

- 組合 Today / Upcoming 首頁資料
- Today = today tasks + today milestone occurrences
- Upcoming = upcoming tasks + upcoming milestone occurrences
