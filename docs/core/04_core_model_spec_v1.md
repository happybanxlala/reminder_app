...
Phase 5J adds account binding foundation and backup legacy transition copy. Settings now shows whether remote-backed shared pack access is local-only, anonymous/unprotected, linked/protected, or missing a current remote session, while Apple / Google / Email binding remains a provider-agnostic unsupported foundation. Manual backup is labeled as legacy local backup and does not restore remote access, replay outbox, pull/import remote snapshots, or store Supabase tokens, sessions, credentials, service-role keys, or plaintext invite codes.
