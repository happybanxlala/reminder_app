...
Phase 5J adds account binding foundation and backup legacy transition copy. Settings now shows whether remote-backed shared pack access is local-only, anonymous/unprotected, linked/protected, or missing a current remote session, while provider binding remains provider-agnostic and production Apple / Google binding stays unsupported. Manual backup is labeled as legacy local backup and does not restore remote access, replay outbox, pull/import remote snapshots, or store Supabase tokens, sessions, credentials, service-role keys, or plaintext invite codes.

Phase 5J.1 adds Email account binding for the current anonymous Supabase session using Email-change OTP codes. The app starts from the existing anonymous remote user, calls Supabase Auth `updateUser(email)` to send a code, verifies with `verifyOTP(type: emailChange)`, and only then marks the existing local user as `identityKind = linked` / `remoteProvider = email` if the Supabase UID is unchanged and the user is no longer anonymous. It does not implement Email sign-in, magic-link login, Apple / Google OAuth, account switching, automatic recovery, snapshot pull/import, outbox flush/retry, local upload, backup replay, or any schema/SQL change. Email addresses, OTP codes, sessions, tokens, credentials, service-role keys, and plaintext invite codes are not persisted in Drift or backup.

Phase 5K adds membership discovery and recovery restore for remote-backed shared packs. A linked/protected remote identity can discover active remote pack memberships through existing RLS, pull each active pack snapshot through the current session, and import or refresh local mirrors through the Phase 5C importer without asking ordinary users to choose remote packs. Anonymous-unprotected, local-only, missing-session, inactive, archived, or removed-member cases fail closed or are skipped. Recovery restore is explicit/manual, refreshes local-derived Home / Widget / Notification surfaces where safe, does not grant access from backup, does not replay outbox, does not flush/retry/push, does not upload local-only packs, does not implement member freshness, and does not add Drift schema, backup schema, or Supabase SQL changes.

Phase 5L adds member sync awareness for remote-backed shared packs as pack-data freshness only. Supabase stores per-member successful import watermarks in `pack_member_sync_states`, and RPCs let active members report a completed local snapshot import or query same-pack active member freshness through `auth.uid()` and RLS. Manual refresh and recovery restore report freshness only after safe local imports; reporting failure becomes a warning and does not roll back local data. The `一起照顧` sheet can show `成員同步狀態` labels without raw remote ids. Phase 5L does not add Drift schema changes, backup schema changes, app-startup recovery, background sync, automatic retry, outbox flush, member monitoring, account switching, resource / stage sync, or full two-way sync.

Phase 5M completes the remote-backed shared pack local-first MVP acceptance pass. It verifies that remote-backed packs can be used as local-first shared packs across creation/join/import, Home, Widget, notification summaries, local outbox complete/undo, manual flush/retry/refresh, Email protection, protected-account recovery, member freshness, and legacy local backup. Phase 5M intentionally stops before background sync, automatic retry, account switching, full conflict resolution, remote item edit/delete/archive sync, resource / stage remote sync, Apple / Google binding, and production-grade cross-device account management.
...
Phase 6G is the final multi-device QA / polish pass for the shared-pack MVP. It keeps the same foreground-only local-first model, fixes readiness gaps found during audit, aligns stale/access-lost copy across Home, widget snapshots, notification summaries, management screens, shared care, and unified activity, and documents a two-device manual QA checklist. Remote-backed item create / basic update / archive / complete / undo must fail closed when the local actor has no remote user id, so the app never reports a successful local-only shared-pack mutation that cannot be flushed. Pull-to-refresh paths flush the current foreground outbox before snapshot fetch/import; if that flush leaves pending, syncing, failed, or conflict mutations for the pack, refresh stops before pulling the remote snapshot to avoid overwriting pending local-first edits. Item outbox flush uses the Phase 6 item RPCs (`create_pack_item_v2`, `update_pack_item`, `archive_pack_item`) and confirms local mappings / metadata after successful remote writes. Phase 6G also completes the Phase 6E Stage SQL draft by defining the app-called Stage RPCs with active-member checks, idempotent `client_mutation_id`, actor preservation, activity events, and soft archive semantics. Phase 6G does not add background sync, app-start sync, realtime auto-import, widget remote CRUD, notification remote sync, new Pack CRUD, member management, ownership transfer, complex conflict UI, a new shared dashboard, or a major architecture rewrite.

Phase 6G Developer Settings include a `Remote sync debug` section for real-device diagnosis only. It shows the current local user id, local `remoteUserId`, Supabase current user id, Supabase session presence, each remote-backed pack's local/remote ids, sync state, current user's remote role/status, unresolved outbox counts, last failed error, and the latest 10 `sync_outbox` rows. The debug actions are foreground-only: `Refresh debug snapshot`, `Force flush pending outbox`, and `Reset stuck syncing to pending` for unresolved remote-backed rows that have been stuck in `syncing` longer than the debug recovery threshold. The device QA flow is: after device A creates a shared-pack item, A's debug panel must show a recent `create_item` row with matching local/remote pack ids, actor ids, and pending/synced status; pressing `Force flush pending outbox` must produce `POST /rest/v1/rpc/create_pack_item_v2` in Supabase Logs. After A edits title/note, the panel must show `update_item`, and force flush must produce `POST /rest/v1/rpc/update_pack_item`. After archive, force flush must produce `POST /rest/v1/rpc/archive_pack_item`. Device B then pull-to-refreshes and imports the new or updated item. If Supabase Logs only show `fetch_pack_invite_state` and no item outbox row exists, save did not enter the remote-backed item repository flow. If the outbox row exists but actor remote id or Supabase session is missing, identity/session blocks sync. If the row is stuck in `syncing` or `failed`, the flush/retry layer is stuck; use reset/force flush and inspect sanitized `lastError`. If the row becomes `synced` and the RPC appears but B still does not update, the remaining issue is B's snapshot fetch/import path.
...
- `FakeAuthRepository` 提供 provider-agnostic fake auth adapter；`signInAnonymously()` 回傳 `fake_supabase_user_<guid>`，Apple / Google / Email fake link 只回傳本機 fake remote identity，Email OTP 測試 flow 會保留同一 fake remote user id 並改成 `remoteProvider = email`。
...
- `signOut` 不刪 local user、不刪 personal pack、不清除 local history。Anonymous users 若未先透過 Email OTP 等可恢復方式保護目前 session，sign out 後可能無法找回同一 remote identity。
