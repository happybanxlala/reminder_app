...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      expect(generated.previousReminderId, reminderId);
      expect(generated.seriesId, original.seriesId);
  ...
  testWidgets(
    'manual batch commit button commits staged recurring reminders',
    (tester) async {
      final db = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
...
Future<int> _seedRecurringReminder(AppDatabase db) async {
  ...
  final createdAt = DateTime(now.year, now.month, now.day).millisecondsSinceEpoch;

  final seriesId = await db.into(db.reminderSeriesEntries).insert(
    ReminderSeriesEntriesCompanion.insert(
      title: 'Weekly task',
      timeBasis: ReminderTimeBasis.countdown,
      notifyStrategy: ReminderNotifyStrategy.inRange,
      remindDays: const drift.Value(2),
